...
import com.turtle.sudoku.bean.LockRequest;
import com.turtle.sudoku.bean.LockResponse;
import com.turtle.sudoku.bean.RequestMessage;
import com.turtle.sudoku.bean.ResponseMessage;
import com.turtle.sudoku.bean.SetValueRequest;
import com.turtle.sudoku.bean.SetValueResponse;
...
public class PKController extends WsController {
	...
	public void chat(ChatRequest request) {
		logger.debug("I'm chating, from {}, msg={}, gameId={}", request.getUsername(), request.getMessage(),
				request.getGameId());
		...
		redisGameService.extendGame(request.getGameId());
		doResponse(topic, response);
	}

	@MessageMapping("/say")
	public void say(ChatRequest request) {
		logger.debug("I'm Saying, from {}, msg={}, gameId={}", request.getUsername(), request.getMessage(),
				request.getGameId());
		System.out.println(request.getRequestType());
		ChatResponse response = new ChatResponse();
		response.setMessage(request.getMessage());
	...
	public void lock(LockRequest request) {
		...
		response.setLockType(request.getLockType());
		String topic = String.format("/topic/game/%d", request.getGameId());
...
//			logger.debug("{} clean value @ position [{}, {}] @ game [{}]", request.getUsername(), request.getX(), request.getY(), request.getGameId());	
		...
		response.setSetType(request.getSetType());
		String topic = String.format("/topic/game/%d", request.getGameId());
